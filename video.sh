...
DEST="/Volumes/shared/Home_Videos"

# Find JPGs
find "$DIR" -type f \( -iname \*.MOV \) | while read f
do
  ...
  # Extract date
  date=`~/Downloads/Image-ExifTool-10.02/exiftool "$f" | grep "Create Date" | head -1 | awk '{print $4}' | awk -F":" '{print $1"-"$2}'`
